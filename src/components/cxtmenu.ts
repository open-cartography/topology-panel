let cxtmenu_defaults = {
    ...
    commands: [ // an array of commands to list in the menu or a function that returns the array
        ...
        { // example command
            ...
            content: 'export', // html/text content to be displayed in the menu
            contentStyle: {}, // css key:value pairs to set the command's css in js if you want
            select: function (cy) { // a function to execute when the command is selected
                exportJson(cy);
            },
...
export default cxtmenu_defaults;

export function exportJson(cy) {
    const json = cy.json();
    const elements = json.elements;
    if (!elements.nodes) {
        return;
    }
    str2file(JSON.stringify(json, undefined, 4), 'sample-graph.json');
}
